...
## Modular Structure Example (hook/tool plugins)

This tree is the layout for a project-local hook/tool plugin: the `hooks/`
and `tools/` directories mirror plugin capabilities. It does NOT apply to
npm-distributed TUI plugins — those keep their source under the package's
`src/` and ship a compiled `dist/` artifact that `tui.json` loads, per
`build-and-release.md` and `tui-reactivity.md`.
...
Extract proactively when a file approaches 150 lines; files SHOULD NOT
exceed 200 lines; files over 300 lines MUST be split.
