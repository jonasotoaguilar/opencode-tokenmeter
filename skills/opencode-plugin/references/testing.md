...
{
  "plugin": ["file:///path/to/opencode/.opencode/plugins/my-plugin/index.ts"],
}
```
...
## Step 2b: Test the Built Dist Output

Before publishing, test the actual artifact, not just the source:

```bash
pnpm build
```

Then load the bundled output directly:
...
This catches bundle errors (missing assets, unresolved imports) that source loading never exercises. Run the full pre-publish gates in `references/build-and-release.md` §4 before publishing.
