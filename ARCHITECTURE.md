...
- **Responsibility**: Presentation-only browser over the existing Project aggregation and `session.messages` on-demand. `eligibility.ts` (`isEligibleProjectPath`) gates every project by sync filesystem checks — `existsSync` + `statSync` directory, `.git` presence, rejects `/`, HOME, or direct child `~/foo`, `parse(root)` roots, and `isSafeDirectory` — so invalid/deleted/non-git and root-level entries never flash (nested `~/projects/foo` stays). `projects-dialog.tsx` is deterministic eligible-only: `project.list` + `project.current` (2 calls at any N) → one usable render ≤100ms, no per-project probes; zero-session eligible projects remain visible. `browser-activity.ts` generation guard + `timeout.ts` bound the single replace; `session-source.ts` for project-aggregated browse now uses legacy directory-scoped `session.list({directory, scope:"project"})` via `resolveSafeWorktree` (V2 project-scoped is instance-scoped, see ADR-0010). `project-detail.ts` keeps legacy directory fetch with pagination guards; `projects.ts` aggregates via same. See ADR-0010 superseding ADR-0009 (58→2 not 58→30). `dialog-shared.tsx`/`projects-dialog.tsx` (`Current Project` / `Projects`, title count-only) / `project-dialog.tsx` (`Current Session` / `Sessions`) / `session-dialog.tsx` render the three `DialogSelect` panels via ONE `api.ui.dialog.replace` at a time (`dialog.tsx` barrel). Host `DialogStack` (`packages/tui/src/ui/dialog.tsx`) calls the previous entry's `onClose` before every `replace` and `clear`; the browser therefore keeps a distinct `close` (idempotent `closed` guard + `dialog.clear()` for user `× Close`) and `onClose` (`if (suppress) return; close()` for host `Escape`/stack lifecycle) with `withSuppress(fn)` wrapping only the single content-update replace so replace-driven `onClose` is a no-op while user `Close`/`Escape` still clear exactly once and late async is blocked by `isActive()`/`closed`; Back returns to that `projectID` without leaking entries.
...
| Eligible-only root without V2 probes (ADR-0010, 2026-08-28) | `eligibility.ts` sync-gates deterministic set (exists + directory + `.git`, not `/`/HOME/`~/foo`); root is `project.list`+`current` (2 calls at any N) → one usable render ≤100ms, zero-session eligible remain visible, no per-project probes; V2 not authoritative cross-project (instance-scoped), detail keeps legacy `session.list({directory})` via `resolveSafeWorktree` | V2 `session.list({project, limit:1})` per-project (30 at N=28), tri-state retain-on-unknown, still instance-scoped |
...
- [ADR-0009: Eligible projects with V2 presence probes and provisional paint](docs/adr/0009-eligible-projects-with-v2-presence-probes.md) (superseded by ADR-0010)
- [ADR-0010: Eligible-only root without V2 presence probes](docs/adr/0010-eligible-only-root-no-probes.md)
