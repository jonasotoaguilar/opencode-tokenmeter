...
- R18: Provide the cross-project usage browser `tokenmeter.browser` (`TokenMeter: Browse Usage`, palette `TokenMeter`, `namespace: "palette"` via `api.keymap.registerLayer`, no `api.command`) opening a native host `DialogSelect` via `api.ui.dialog.replace` (ONE replace at a time, once-guarded close, no recursive replace). Navigation: `Projects` (all known projects with lifetime totals from the existing Project aggregation — live `session.list` + deleted SQLite aggregate — pinned current first, sorted recent, no per-project model breakdown) → `Project detail` (selected project's aggregate, period, last activity, session list pinned current first, sorted recent, with `← Back to projects`) → `Session detail` (selected session's title/id, lifetime tokens/cost resolved via existing `resolveCost`/`TokenMeter` semantics — reported non-zero wins, OpenAI zero estimates if current pricing exists else safe-zero — last activity, provider groups with models underneath grouped by `providerID` then `modelID`, sorted providers/models by spend descending, costs via `fmtCost`, short model labels last segment but provider id exact, current session visually marked with `★`, loading/empty/error states, `← Back to project` returning to that `projectID` without leaking stack entries). Project-level breakdown is presentation-only for the opened session via `session.messages` on-demand; no second usage truth is introduced.
