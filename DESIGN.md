...
**Agent entries and scrollbox:** each agent entry is a `↳`-indented header plus metric rows indented four columns (`AGENT_METRIC_INDENT`) aligned under the agent name; inside the scrollbox every row budget reserves one scrollbar column. With the section expanded, ALL groups render inside the scrollbox (viewport 6 — up to three collapsed entries) and scroll; one wheel gesture snaps to the next/previous agent header using the real entry heights (collapsed 2, expanded compact 4, expanded precise 6), clamped to the content bounds; toggling an agent keeps its header visible while the detail opens downward. Nothing is sliced and no clipped cue is rendered.
