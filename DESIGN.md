...
**Agent entries and scrollbox:** each agent entry is a `↳`-indented header plus metric rows indented four columns (`AGENT_METRIC_INDENT`) aligned under the agent name; inside the scrollbox every row budget reserves one scrollbar column. With the section expanded, ALL groups render inside the scrollbox (viewport 6 — up to three collapsed entries) and scroll; one wheel gesture always advances exactly two rows, clamped to the content bounds; toggling an agent auto-scrolls the minimum distance needed to show its full expanded entry (header + all detail rows — 4 rows in compact, 6 in precise), and collapsing clamps back with no blank rows. Nothing is sliced and no clipped cue is rendered.
