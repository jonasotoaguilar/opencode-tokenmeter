...
**Metric rows (shared by Session, Project, and every agent entry):** the primary token+cost line is `<total> tokens · $<spend>` (`text` with the `$amount` in `error`; the word `spent` is never rendered); the secondary rows are `<input> input · <output> output` and `<reason> reason · <cache> cache` in the derived detail tone, where real output = raw output + raw reasoning computed exactly once and the reasoning label is exactly `reason`. In `compact` number mode exactly three labeled rows render (primary + paired input/output + paired reason/cache); in `precise` mode exactly five single-metric rows (tokens+cost, input, output, reason, cache). Cache renders a combined single value, or `R<read>|W<write>` in separated mode with zero sides omitted and `0` when both are zero.
...
| `secondary-row` | derived detail tone | Labeled `input`/`output`/`reason`/`cache` rows; nested 2 columns |
...
- Don't: use emoji or Nerd Font PUA codepoints as functional icons — the disclosure glyphs are plain Unicode (`▶`/`▼`/`↳`).
