...
describe("browser session-source", () => {
  test("fetchSessionsForBrowse returns empty when no safe directory", async () => {
    const api = {
  ...
  test("fetchSessionsForBrowse filters by projectID via directory", async () => {
    const dir = "/tmp"
    const api = {
      state: { path: { directory: dir, state: dir } },
      client: {
        project: {
          list: async () => ({ data: [{ id: "proj1", worktree: dir }] }),
        },
        session: {
  ...
  test("fetchSessionsForProject delegates", async () => {
    ...
    const api = {
      ...
      client: {
        project: {
          list: async () => ({ data: [{ id: "p1", worktree: dir }] }),
        },
        session: {
