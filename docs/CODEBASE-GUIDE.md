...
| [src/tokenmeter/browser/](../src/tokenmeter/browser/) | Cross-project browser: eligible provisional Projects (`eligibility.ts`), V2 presence probes (`projects-dialog.tsx` + `probeHasSessionsV2`), then final sync via bounded concurrency/timeouts, generation guard, and host `DialogStack` lifecycle (`replace`/`clear` → previous `onClose`; browser separates host `onClose` from user `close` and suppresses `onClose` only during content-update replaces via `withSuppress`) — Projects (`Current Project`/`Projects`) → Project detail (`Current Session`/`Sessions`) → Session detail via ONE-replace dialogs | `eligibility.ts`, `constants.ts`, `timeout.ts`, `concurrency.ts`, `browser-activity.ts`, `is-safe-directory.ts`, `types.ts`, `directories.ts`, `session-source.ts`, `projects.ts`, `project-detail.ts`, `session-detail.ts` (+`session-info.ts`, `session-messages.ts`, `session-tree.ts`, `session-fallback.ts`), `dialog-shared.tsx`, `projects-dialog.tsx`, `project-dialog.tsx`, `session-dialog.tsx`, `dialog.tsx` |
...
- `docs/adr/` — architecture decision records (build, reconcile, kv, external packages, width, cost fallback, eligible V2 probes — see [ADR-0008](adr/0008-openai-cost-fallback-with-models-dev.md) (supersedes 0007) and [ADR-0009](adr/0009-eligible-projects-with-v2-presence-probes.md) via [ARCHITECTURE.md](../ARCHITECTURE.md)).
