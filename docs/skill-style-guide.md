# LLM-first Skill Style Guide — opencode-tokenmeter

This is the **normative** authoring guide for skills in this repo (the bundled skill lives at `skills/opencode-plugin/`). Use it when creating or refactoring skills here. A skill is a **runtime instruction contract for an LLM**, not human-facing documentation: it tells the model when to activate, what rules are non-negotiable, how to decide, what to do, and what to return.
...
- The registry (`gentle-ai skill-registry refresh`, output `.atl/skill-registry.md`) indexes skills; it does not summarize or rewrite them.
...
- The authoritative plugin-development skill in this repo is `skills/opencode-plugin/SKILL.md` — do not create a competing skill; extend that one or reference it.
- New or updated skills MUST be registered via the skill-registry procedure, never edited by hand.
- Skill content follows the persona scope of this repo's AGENTS.md: artifacts are neutral English, never conversational.
